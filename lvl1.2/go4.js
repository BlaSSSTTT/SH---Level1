...
function readHttpLikeInput(){
   ...
   function processHttpRequest($method, $uri, $headers, $body) {
       ...
       if($method!=="POST"){
            ...
            $body = "not found";
       }else  if(!$uri.startsWith("/api/checkLoginAndPassword")){
            statusCode=400;
            statusMessage = "Bad Request";
            $body = "not found";
        }else{
            let logAnsPass = $body.split("&");
            let log = logAnsPass[0].replace('login=', "");
            let pass = logAnsPass[1].replace('password=', "");
            const searchStr = log + ":" + pass;
            const fs = require("fs");

            try {
                const passwords = fs.readFileSync("passwords.txt", "utf8");

                if (passwords.includes(searchStr)) {
